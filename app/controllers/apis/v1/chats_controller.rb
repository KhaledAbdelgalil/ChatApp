...
module Apis
  module V1
    # Chats Controller : contains endpoints(actions) needed for the chats model
    class ChatsController < ApplicationController
      def chats
        app = App.find_by_token(params[:app_token])
        if app
          if app.chats_count != 0
            render json: { status: 'SUCCESS', message: 'Loaded chats', data: app.chats.as_json(only: %i[chat_number created_at]) },
                   status: :ok
          else
