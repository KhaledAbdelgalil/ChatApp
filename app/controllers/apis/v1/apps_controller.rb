...
module Apis
  module V1
    # Apps Controller : contains endpoints(actions) needed for the app model
    class AppsController < ApplicationController
      def show_all
        apps = App.all.as_json(only: %i[name token created_at])
        if apps.count != 0
      ...
      def show_one
        ...
        if app
          render json: { status: 'SUCCESS', message: 'Loaded app', data: app.as_json(only: %i[token name created_at]) },
                 status: :ok
        else
      ...
      def create_app
        ...
        if app.save
          render json: { status: 'SUCCESS', message: 'App is created', data: app.as_json(only: %i[token name created_at]) },
                 status: :ok
        else
          render json: { status: 'ERROR', message: 'App not created', data: app.errors },
      ...
      def update
        ...
        if app
          if app.update(app_params)
            ...
            render json: { status: 'ERROR', message: 'Application not updated', data: app.errors },
                   status: :unprocessable_entity
          end
        else
          render json: { status: 'Failed', message: ErrorController.invalid_token(params[:app_token].to_s) }
